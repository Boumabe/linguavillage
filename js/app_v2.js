...
function _initSession() {
  // Vérifier si c'est la toute première session (intro narrative)
  var isFirstEver = false;
  try { isFirstEver = !localStorage.getItem('lv_intro_seen'); } catch(e) {}

  if (isFirstEver && window.LV_WORLD && typeof window.LV_WORLD.playIntroStory === 'function') {
    var nl = (window.S && S.nativeLang) || 'fr';
    window.LV_WORLD.playIntroStory(nl, function() {
      try { localStorage.setItem('lv_intro_seen', '1'); } catch(e) {}
      _goToMenu();
    });
  } else {
    _goToMenu();
...
function _goToMenu() {
  ...
  try {
    if (window.LV_ANIM) {
      window.LV_ANIM.checkJ1Retention();
      window.LV_ANIM.checkJ7Retention();
      window.LV_ANIM.checkJ30Retention();
      window.LV_ANIM.scheduleSmartReminder();
    }
  } catch(e) {}
