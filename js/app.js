...
// =================================================================
// PHRASES DATA — 1000 everyday phrases (version réduite pour lisibilité)
// =================================================================
const PHRASES_DATA = {
  quotidien:{
...
const GRAMMAR_DATA = {
  present:{
    ...
    explanation:{
      fr:'Le présent décrit ce qui se passe <strong>maintenant</strong> ou ce qui est <em>habituel/régulier</em>.',
      en:'The present tense describes what is happening <strong>now</strong> or what is <em>habitual/regular</em>.',
    },
    formula:{fr:'Sujet + Verbe (conjugué)',en:'Subject + Verb (conjugated)'},
    examples:[
      {n:'Je mange une pomme chaque matin.',t:{en:'I eat an apple every morning.'}},
      {n:'Il travaille dans un bureau.',t:{en:'He works in an office.'}},
      {n:'Nous parlons français.',t:{en:'We speak French.'}},
    ]
  },
};
...
const LOC_NAMES = {
  ...
  cinema:{fr:'Cinéma',ht:'Sinema',en:'Cinema',es:'Cine',de:'Kino',ru:'Кино',zh:'电影',ja:'映画'},
};
...
const LOC_DESC = {
  ...
  cinema:{fr:'Films et vidéos',en:'Movies & videos',ht:'Fim ak videyo',es:'Películas',de:'Filme',ru:'Кино',zh:'电影',ja:'映画'},
};

const LOCATIONS = [
  {id:'church',emoji:'⛪',color:'#6a50a8',x:0.42,y:0.38,w:0.10,h:0.12,npcs:[
    {id:'pastor',emoji:'🧑‍⚖️',name:'Morgan',role:{fr:'Pasteur·e',en:'Pastor',ht:'Pastè',es:'Pastor/a',de:'Pastor/in',ru:'Пастор',zh:'牧师',ja:'牧師'},ctx:'Tu es Morgan, pasteur bienveillant. Tu parles avec respect et solennité.'},
  ...
  {id:'school',emoji:'🏫',color:'#3a7aaa',x:0.53,y:0.44,w:0.10,h:0.12,npcs:[
    {id:'teacher',emoji:'🧑‍🏫',name:'Robin',role:{fr:'Professeur·e',en:'Teacher',ht:'Pwofesè',es:'Profesor/a',de:'Lehrer/in',ru:'Учитель',zh:'老师',ja:'先生'},ctx:'Tu es Robin, professeur patient. Tu corriges doucement.'},
    {id:'student',emoji:'🧒',name:'Charlie',role:{fr:'Élève',en:'Student',ht:'Elèv',es:'Estudiante',de:'Schüler/in',ru:'Ученик',zh:'学生',ja:'生徒'},ctx:'Tu es Charlie, élève curieux.'}]},
  {id:'market',emoji:'🛒',color:'#2a8a50',x:0.31,y:0.44,w:0.10,h:0.12,npcs:[
    ...
    {id:'cashier',emoji:'💰',name:'Quinn',role:{fr:'Caissier·ère',en:'Cashier',ht:'Kesye',es:'Cajero/a',de:'Kassierer/in',ru:'Кассир',zh:'收银员',ja:'レジ係'},ctx:'Tu es Quinn, caissier méthodique.'}]},
  ...
  {id:'tavern',emoji:'🍺',color:'#8a4a10',x:0.20,y:0.34,w:0.10,h:0.12,npcs:[
    ...
    {id:'regular',emoji:'🎲',name:'Sky',role:{fr:'Client·e',en:'Regular',ht:'Kliyan',es:'Cliente',de:'Stammgast',ru:'Завсегдатай',zh:'常客',ja:'常連'},ctx:'Tu es Sky, habitué décontracté.'}]},
  ...
  {id:'cinema',emoji:'🎬',color:'#8a2080',x:0.42,y:0.60,w:0.10,h:0.12,npcs:[
    {id:'projectionist',emoji:'🎥',name:'Milly',role:{fr:'Projectionniste',en:'Projectionist',ht:'Pwojeksyonis',es:'Proyeccionista',de:'Vorführerin',ru:'Киномеханик',zh:'放映员',ja:'映写技師'},ctx:'Tu es Milly, projectionniste passionnée.'}]},
];
...
function buildWeatherFX(w){
  ...
  if(w==='rain'){
    for(let i=0;i<60;i++){
      ...
      d.style.cssText=`left:${Math.random()*110-5}%;height:${h}px;top:-${h}px;animation-duration:${0.4+Math.random()*0.4}s;animation-delay:${Math.random()*2}s;opacity:${0.3+Math.random()*0.4}`;
      o.appendChild(d);
    }
  } else if(w==='snow'){
    for(let i=0;i<40;i++){
...
function drawVillage(){
  ...
  const rings=[{r:W*0.46, color:'rgba(160,130,80,0.25)'},{r:W*0.32, color:'rgba(160,130,80,0.30)'},{r:W*0.20, color:'rgba(160,130,80,0.38)'},{r:W*0.10, color:'rgba(160,130,80,0.50)'}];
  rings.forEach(ring=>{ctx.beginPath();ctx.arc(cx,cy,ring.r,0,Math.PI*2);ctx.fillStyle=ring.color;ctx.fill();});
  
  // Murs
  const wallRadii=[W*0.46, W*0.32, W*0.20, W*0.10];
  const wallColors=['#8a7040','#9a8050','#aa9060','#c0a870'];
  wallRadii.forEach((r,i)=>{ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.strokeStyle=wallColors[i];ctx.lineWidth=Math.max(3,W*0.012);ctx.stroke();});
  
  // Chemins
  const pathColor='rgba(180,150,90,0.6)';
  const pathW=Math.max(6,W*0.022);
  [0, Math.PI/2, Math.PI, Math.PI*1.5].forEach(angle=>{
    const x1=cx+Math.cos(angle)*W*0.10, y1=cy+Math.sin(angle)*W*0.10;
    const x2=cx+Math.cos(angle)*W*0.52, y2=cy+Math.sin(angle)*W*0.52;
    ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);
    ctx.strokeStyle=pathColor;ctx.lineWidth=pathW;ctx.lineCap='round';ctx.stroke();
  });
...
function drawLoc(loc,x,y,w,h,hov){
  ...
  if(hov){ctx.beginPath();ctx.arc(bx,by,r,0,Math.PI*2);ctx.strokeStyle='#FFD700';ctx.lineWidth=2.5;ctx.stroke();}
  else{ctx.beginPath();ctx.arc(bx,by,r,0,Math.PI*2);ctx.strokeStyle=hexA(darken(loc.color),0.8);ctx.lineWidth=1.5;ctx.stroke();}
  if(night){ctx.fillStyle='rgba(255,215,100,0.6)';ctx.fillRect(bx-r*0.3, by-r*0.15, r*0.22, r*0.22);ctx.fillRect(bx+r*0.08,by-r*0.15, r*0.22, r*0.22);}
...
function openLoc(loc){
  ...
  setTimeout(()=>{if(typeof openMissionsPanel==='function') openMissionsPanel(loc.id);},400);
}
...
async function npcOpen(){
  ...
  try{
    ...
    const reply=d.reply||`Bonjour ${S.playerName}!`;
    addClickableMsg('npc',npc.emoji,reply);S.chatHistory.push({role:'assistant',content:reply});
  }catch(e){removeTyping();addClickableMsg('npc',npc.emoji,`Bonjour ${S.playerName}!`);}
...
function loadVocab(catKey){
  ...
  catsBar.innerHTML=cats.map(k=>{const a=k===catKey?' active':'';const icon=VOCAB[k].icon||'📖';const label=VOCAB[k][S.nativeLang]||VOCAB[k].fr;return `<button class="vcat${a}" onclick="loadVocab('${k}')">${icon} ${label}</button>`;}).join('');
  ...
  const vocabRowsHTML=words.map(w=>{
    ...
    return `<div class="vocab-item"><span class="vi-native">${nativeText}</span><span class="vi-target"><span class="vi-word">${showNative?chars:''}</span>${romanSpan}${altWord}</span><button class="vi-listen" onclick="speakW('${escapedChars}')">🔊</button></div>`;
  }).join('');
  const catLabel=cat[S.nativeLang]||cat.fr||"";
  document.getElementById('vocabList').innerHTML=`<div class="cat-header">${catLabel} — ${words.length} mots</div>${vocabRowsHTML}`;
}
...
function loadPhrases(catKey){
  ...
  document.getElementById('phraseCats').innerHTML=cats.map(k=>{const a=k===catKey?' active':'';const icon=PHRASES_DATA[k].icon||'';const label=PHRASES_DATA[k][S.nativeLang]||PHRASES_DATA[k].fr;return `<button class="pcat${a}" onclick="loadPhrases('${k}')">${icon} ${label}</button>`;}).join('');
  ...
  document.getElementById('phraseList').innerHTML=cat.items.map(p=>{
    ...
    const escapedChars=chars.replace(/'/g,"\\'");
    return `<div class="phrase-item"><div class="pi-native">${nativeText}</div><div class="pi-target">${showNative?chars:target}</div>${romanHtml}${structHtml}<div class="pi-actions"><button class="pi-btn" onclick="speakW('${escapedChars}')">🔊</button><button class="pi-btn" onclick="copyPhrase('${escapedChars}')">📋</button></div></div>`;
  }).join('');
}

function copyPhrase(t){if(navigator.clipboard){navigator.clipboard.writeText(t).then(()=>showNotif('📋 Copié !')).catch(()=>showNotif('❌ Erreur de copie'));}else{const textArea=document.createElement("textarea");textArea.value=t;document.body.appendChild(textArea);textArea.select();document.execCommand('copy');document.body.removeChild(textArea);showNotif('📋 Copié !');}}

function loadGrammar(catKey){
  const cats=Object.keys(GRAMMAR_DATA);
  document.getElementById('grammarCats').innerHTML=cats.map(k=>{const a=k===catKey?' active':'';const icon=GRAMMAR_DATA[k].icon||'';const label=GRAMMAR_DATA[k][S.nativeLang]||GRAMMAR_DATA[k].fr;return `<button class="gcat${a}" onclick="loadGrammar('${k}')">${icon} ${label}</button>`;}).join('');
  ...
  const gramRowsHTML=(cat.examples||[]).map(ex=>{
    ...
    const escapedChars=chars.replace(/'/g,"\\'");
    return `<div class="gram-ex"><span class="gram-ex-native">${nativeText}</span><span class="gram-ex-target">${showNative?chars:target}${romanSpan}<button style="background:none;border:none;cursor:pointer;color:var(--dim);margin-left:4px" onclick="speakW('${escapedChars}')">🔊</button></span></div>`;
  }).join('');
  const count=cat.items?.length||cat.examples?.length||0;
  const title=cat[nl]||cat.fr||'';
  document.getElementById('grammarBody').innerHTML=`<div class="gram-section"><div class="gram-title">${cat.icon||''} ${title}<span class="gram-tag">${count} exemples</span></div><div class="gram-explanation">${expl}</div>${formula?`<div class="gram-formula">${formula}</div>`:''}<div class="gram-examples">${gramRowsHTML}</div></div>`;
}

function openDict(){
  dictFromScreen=document.querySelector('.screen.active')?.id||'screen-menu';
  document.getElementById('dictInput').value='';
  document.getElementById('dictResult').innerHTML=`<div class="dict-empty"><div class="dict-empty-icon">🔤</div><div>Tapez un mot ou une phrase</div></div>`;
...
async function searchDict(){
  ...
  const isCJK=['zh','ja','ru'].includes(S.targetLang);
  const showRoman=isCJK&&S.scriptPref!=='native';
  const prompt=dictMode==='phrase'?`Traduis cette phrase du ${nln[S.nativeLang]||'français'} vers le ${nln[S.targetLang]||'anglais'}: "${q}". JSON: {"translation":"...","roman":"romanisation si applicable","grammar":"structure","example":"exemple simple"}`:`Traduis le mot "${q}" du ${nln[S.nativeLang]||'français'} vers le ${nln[S.targetLang]||'anglais'}. JSON: {"translation":"...","roman":"romanisation","grammar":"nom/verbe/adj...","example":"phrase exemple"}`;
  try{
    ...
    const d=await r.json();
    let p;
    try{p=JSON.parse((d.reply||'{}').replace(/```json|```/g,'').trim());}catch{p={translation:d.reply||q,roman:'',grammar:'',example:''};}
    const hist=dictHistory.slice(1,9);
    const escapedTranslation=(p.translation||'').replace(/'/g,"\\'");
    let histHTML='';
    if(hist.length){histHTML='<div style="font-size:0.65rem;color:var(--dim);letter-spacing:2px;text-transform:uppercase;margin:15px 0 8px 0">Historique</div><div class="dict-chips">'+hist.map(h=>`<span class="dict-chip" onclick="searchDictWord('${h.replace(/'/g,"\\'")}')">${h}</span>`).join('')+'</div>';}
...
function addCEFRIndicator(){
  ...
  if(totalXP<300){currentLevel="A1";nextLevel="A2";progressPercent=Math.min(100,Math.floor((totalXP/300)*100));levelColor="#4ecf70";levelIcon="🌱";}
  else if(totalXP<800){currentLevel="A2";nextLevel="B1";progressPercent=Math.min(100,Math.floor(((totalXP-300)/500)*100));levelColor="#4a9eff";levelIcon="🌟";}
  else if(totalXP<1500){currentLevel="B1";nextLevel="B2";progressPercent=Math.min(100,Math.floor(((totalXP-800)/700)*100));levelColor="#ff9f43";levelIcon="🏆";}
  else if(totalXP<2500){currentLevel="B2";nextLevel="C1";progressPercent=Math.min(100,Math.floor(((totalXP-1500)/1000)*100));levelColor="#e040fb";levelIcon="👑";}
  else{currentLevel="C1";nextLevel=null;progressPercent=100;levelColor="#ff6b6b";levelIcon="🏅";}
  ...
  indicator.onclick=function(){showNotif('🗺️ Niveau '+currentLevel+' → '+(nextLevel||'🏆 Maître !')+' ('+progressPercent+'%)');};
  indicator.innerHTML='<span style="font-size:0.85rem;">'+levelIcon+'</span><span style="font-weight:800;color:'+levelColor+'">'+currentLevel+'</span><div style="width:40px;height:4px;background:rgba(255,255,255,0.2);border-radius:2px;overflow:hidden;"><div style="width:'+progressPercent+'%;height:100%;background:'+levelColor+';border-radius:2px;"></div></div>'+(nextLevel?'<span style="font-size:0.6rem;color:var(--dim);">→ '+nextLevel+'</span>':'🏆');
  hud.appendChild(indicator);
}

const originalGoVillage=window.goVillage;
window.goVillage=function(){originalGoVillage();setTimeout(function(){addCEFRIndicator();},100);};

// =================================================================
// INIT DOM — tous les listeners après chargement complet
// =================================================================
window.addEventListener('DOMContentLoaded', function(){
  ...
  // Langue maternelle
  document.querySelectorAll('[data-native]').forEach(t=>{t.addEventListener('click',()=>{document.querySelectorAll('[data-native]').forEach(x=>x.classList.remove('sel'));t.classList.add('sel');S.nativeLang=t.dataset.native;applyUI(S.nativeLang);document.getElementById('step2').style.display='block';['step3','step4','playBtn'].forEach(id=>{var el=document.getElementById(id);if(el){el.style.display='none';if(el.tagName==='BUTTON')el.disabled=true;}});document.querySelectorAll('[data-lang]').forEach(o=>{o.classList.toggle('disabled',o.dataset.lang===S.nativeLang);if(o.dataset.lang===S.nativeLang)o.classList.remove('sel');});});});

  // Nom
  var inputName=document.getElementById('inputName');if(inputName)inputName.addEventListener('input',function(){var hasValue=this.value.trim().length>0;document.getElementById('step3').style.display=hasValue?'block':'none';document.getElementById('step4').style.display='none';document.getElementById('playBtn').style.display='none';});

  // Langue cible
  document.querySelectorAll('[data-lang]').forEach(o=>{o.addEventListener('click',function(){if(o.classList.contains('disabled'))return;document.querySelectorAll('[data-lang]').forEach(x=>x.classList.remove('sel'));o.classList.add('sel');S.targetLang=o.dataset.lang;var cjk=['zh','ja','ru'].includes(S.targetLang);var playBtn=document.getElementById('playBtn');if(cjk){document.getElementById('step4').style.display='block';var lb={zh:{n:'你好',r:'Nǐ hǎo'},ja:{n:'こんにちは',r:'Konnichiwa'},ru:{n:'Привет',r:'Privyet'}};document.getElementById('sc-n').textContent=lb[S.targetLang].n;document.getElementById('sc-r').textContent=lb[S.targetLang].r;playBtn.style.display='none';playBtn.disabled=true;}else{S.scriptPref='both';document.getElementById('step4').style.display='none';playBtn.style.display='block';playBtn.disabled=false;}});});

  // Bouton Commencer
  var playBtn=document.getElementById('playBtn');if(playBtn)playBtn.addEventListener('click',function(){S.playerName=document.getElementById('inputName').value.trim();if(!S.playerName||!S.nativeLang||!S.targetLang){showNotif('⚠️ Complétez tous les champs !');return;}startMenu();});

  // Dialogue Enter
  var dialInput=document.getElementById('dialInput');if(dialInput)dialInput.addEventListener('keydown',function(e){if(e.key==='Enter')sendMsg();});

  // Fermeture popup
  document.addEventListener('click',function(e){var p=document.getElementById('wordPopup');if(p&&p.classList.contains('show')&&!p.contains(e.target)&&!e.target.classList.contains('clickable-word'))closeWordPopup();});

  // Recherche vocabulaire
  var vSearch=document.getElementById('vocabSearch');if(vSearch)vSearch.oninput=function(){var activeBtn=document.querySelector('.vcat.active');if(activeBtn){var activeIdx=Array.from(document.querySelectorAll('.vcat')).indexOf(activeBtn);var catKey=Object.keys(VOCAB)[activeIdx];loadVocab(catKey);}};

  // Dictionnaire Enter
  var dictInput=document.getElementById('dictInput');if(dictInput)dictInput.addEventListener('keydown',function(e){if(e.key==='Enter')searchDict();});
});
